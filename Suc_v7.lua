...
CreateNode("TextLabel", {Size = UDim2.new(1, 0, 0, 25), Position = UDim2.new(0, 5, 0, 0), BackgroundTransparency = 1, Text = "> MayChemXeoCan_Middleware", TextColor3 = Color3.fromRGB(0, 255, 100), Font = Enum.Font.Code, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left}, Core.MainBox)
CreateNode("TextLabel", {Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "System Overrided. BananaCat Blinded.", TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.GothamBold, TextSize = 13}, Core.AlertBox)
...
t_spawn(function()
    while t_wait(0.5) do
        pcall(function()
            ...
            if t and t.Parent and t:FindFirstChild("Humanoid") and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
                local tHrp = t:FindFirstChild("HumanoidRootPart") or t:FindFirstChild("Ghosted_HRP")
                if not tHrp then return end
                
                if t.Name ~= cTarg then
                    cTarg = t.Name; lHP = t.Humanoid.Health; fH = nil
                    if t:FindFirstChild("Ghosted_HRP") then t.Ghosted_HRP.Name = "HumanoidRootPart" end
                else
                    ...
                    if d > 15000 or (fH and tick() - fH > 18) then
                        BL[cTarg] = tick(); getgenv().CurrentTarget = nil
                        if tHrp.Name == "HumanoidRootPart" then tHrp.Name = "Ghosted_HRP" end
                        Logger.Write("Ghost_Logic", "Banana Blinded -> " .. cTarg, Color3.fromRGB(255, 50, 50))
                        cTarg = nil
                    end
                end
            else
